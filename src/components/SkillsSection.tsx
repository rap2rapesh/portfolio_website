  ...
  icon
    ...
    <TableCell className="w-3/4">
      <div className="flex flex-wrap gap-2">
        {skills.map((skill, index) => <Badge key={index} className="bg-gray-100 hover:bg-portfolio-primary hover:text-white text-gray-700 py-2 px-5 text-xs rounded-full transition-colors">
            {skill}
          </Badge>)}
      </div>
    </TableCell>
  ...
  return <section id="skills" className="bg-gray-50 my-0 py-0">
      <div className="container mx-0 px-[16px] rounded my-0">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-portfolio-dark py-0 my-[30px]">
        ...
        <div className="mb-16 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <Table>
            ...
            <TableBody>
              {skillCategories.map((category, index) => <SkillCategory key={index} title={category.title} skills={category.skills} icon={category.icon} />)}
            </TableBody>
          </Table>
        </div>
      </div>
